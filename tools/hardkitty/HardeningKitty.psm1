Function Invoke-HardeningKitty {
    ...
    # Check if the user has admin rights, stop running if not
    If (-not($IsAdmin)) {
    ...
    If ($Log -and $LogFile.Length -eq 0) {
        $LogFile = $ListName + "-$FileDate.log"
    }
    If ($Report -and $ReportFile.Length -eq 0) {
        $ReportFile =  $ListName + "-$FileDate.csv"
    }
    If ($Backup -and $BackupFile.Length -eq 0) {
        $BackupFile = $ListName + "-$FileDate.csv"
    }
    $ReportAllResults = @()
    $BackupAllResults = @()
    ...
    If ($Mode -eq "Audit" -or $Mode -eq "Config") {
        ...
        If ($FileFindingList.Length -eq 0) {
            ...
            $DefaultList = "$CurrentLocation\lists\machine.csv"
    ...
    Elseif ($Mode -eq "HailMary") {
        ...
        # Limit the settings by severity rating
        $FindingList = Import-Csv -Path $FileFindingList -Delimiter ","
        $LastCategory = ""
        ...
        ForEach ($Finding in $FindingList) {
            ...
            # 
            # Level of Hardening
            # This is TODO where you can select what level of hardening to do.  
            # Thinking about using the 'severity' column in the csv, this is for the scoring?
